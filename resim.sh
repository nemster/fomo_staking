...
for i in $(seq 1 500)
do
	resim call-method ${component} airdrop ${coin2}:2 >$OUTPUTFILE || ( cat $OUTPUTFILE ; exit 1 )
	echo -n .
done
echo 500 airdrops of 2 COIN2 each done
...
echo "NFT #${nft_id}# unstaked, ignoring coin $coin3, ${withdraw_events} WithdrawEvents, ${deposit_events} DepositEvents, XRD ${fee_paid} fee payd "
...
echo "FOMO received: ${fomo_received} (should be 166), COIN2 received: ${coin2_received} (should be 666), COIN3 received: ${coin3_received} (should be zero)"
...
export deposit_events=$(grep DepositEvent $OUTPUTFILE | wc -l)
export fee_paid=$(grep -A 1 resource_sim1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxakj8n3 $OUTPUTFILE | grep 'Change: -' | cut -d - -f 2)
echo "NFT #${nft_id}# unstaked, ${withdraw_events} WithdrawEvents, ${deposit_events} DepositEvents, XRD ${fee_paid} fee payd "
export fomo_received=$(grep -A 1 "ResAddr: $fomo" $OUTPUTFILE | tail -n 1 | cut -d : -f 2)
export coin2_received=$(grep -A 1 "ResAddr: $coin2" $OUTPUTFILE | tail -n 1 | cut -d : -f 2)
export coin3_received=$(grep -A 1 "ResAddr: $coin3" $OUTPUTFILE | tail -n 1 | cut -d : -f 2)
echo "FOMO received: ${fomo_received} (should be 83), COIN2 received: ${coin2_received} (should be 333), COIN3 received: ${coin3_received} (should be 166)"
...
export deposit_events=$(grep DepositEvent $OUTPUTFILE | wc -l)
export fee_paid=$(grep -A 1 resource_sim1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxakj8n3 $OUTPUTFILE | grep 'Change: -' | cut -d - -f 2)
echo "NFT #${nft_id}# unstaked, WithdrawEvents, ${deposit_events} DepositEvents, XRD ${fee_paid} fee payd "
export fomo_received=$(grep -A 1 "ResAddr: $fomo" $OUTPUTFILE | tail -n 1 | cut -d : -f 2)
export coin2_received=$(grep -A 1 "ResAddr: $coin2" $OUTPUTFILE | tail -n 1 | cut -d : -f 2)
export coin3_received=$(grep -A 1 "ResAddr: $coin3" $OUTPUTFILE | tail -n 1 | cut -d : -f 2)
